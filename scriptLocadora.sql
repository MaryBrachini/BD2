...
INSERT INTO clienteAlugaExemplarFilme VALUES (1,1,1,'11/04/2005','12/04/2005');
-- (idCliente,codigoFilme, nroExemplar, dtaRet,dtaDev)
...
--Inner Join
select nome, a.idAtor, codFilme from ator a inner join atorEstrelaFilme aef 
on a.idAtor = aef.idAtor;

--Ex1
select a.nome, f.nome from ator a, atorEstrelaFilme aef, filme f 
where a.idAtor = eaf.idAtor
and aef.codFilme = f.codFilme and f.nome like '%a';

select a.nome, f.nome from ator a inner join atorEstrelaFilme aef on a.idAtor = eaf.idAtor
inner join filme f on f.codFilme aef.codFilme;

--Ex2
select nome, telefone from cliente c inner join telefoneCliente t on c.codCliente = t.codCliente;

--Ex3
select a.nome, genero from ator a inner join atorEstrelaFilme aef 
on a.idAtor = aef.idAtor
inner join filme f on f.codFilme = aef.codFilme
and genero = 'Drama';

--Ex4
select nome, genero from filme where genero = 'Ação' or genero = 'Aventura' ;

--Ex5
select nome, dtaNasc from ator where dtaNasc >='10/10/1970' and dtaNasc <='10/10/1990';

--Ex6
select genero, count(genero) from filme group by genero having count(genero) >3 order by genero; 
